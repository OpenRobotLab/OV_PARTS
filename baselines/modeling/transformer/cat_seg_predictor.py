...
class CATSegPredictor(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.text_features = self.class_embeddings(self.train_class_texts, prompt_templates, clip_model).permute(1, 0, 2).float()
    ...
    def forward(self, x, vis_guidance, test_text=None):
        ...
        if self.training:
            ...
            text = self.text_features_test[[self.test_class_texts.index(t) for t in test_text]]
        else:
    ...
    def class_embeddings(self, classnames, templates, clip_model):
        ...
        if self.bg_on:
            ...
            zeroshot_weights = torch.cat([zeroshot_weights, non_object_text_features.unsqueeze(0).repeat(len(zeroshot_weights),1,1).cuda()], dim=-2)
