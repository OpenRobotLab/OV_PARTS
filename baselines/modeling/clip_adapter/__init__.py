...
from .text_prompt import (
    ...
    LearnablePartPromptExtractor,
    build_modified_clip_model
)
from .adapter import ClipAdapter, MaskFormerClipAdapter, PerPixelClipAdapter, MaskFormerObjPartClipAdapter
...
def build_prompt_learner(cfg):
    ...
    elif cfg.PROMPT_LEARNER == "learnable_obj_part":
        clip_model = build_modified_clip_model(cfg.CLIP_MODEL_NAME)
        prompt_learner = LearnablePartPromptExtractor(
            prompt_dim=cfg.PROMPT_DIM,
            prompt_shape=cfg.PROMPT_SHAPE,
        )
        prompt_learner.init_buffer(clip_model)
        prompt_learner.init_prompt(clip_model)
        if cfg.PROMPT_CHECKPOINT != "":
            checkpoint = torch.load(cfg.PROMPT_CHECKPOINT, map_location="cpu")["model"]
            missing, unexpected = prompt_learner.load_state_dict(
                {
                    k[len("prompt_learner")+1:]: v
                    for k, v in checkpoint.items()
                    if "prompt_learner" in k
                },
                strict=False,
