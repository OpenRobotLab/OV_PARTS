...
class CLIPSegForImageSegmentation(CLIPSegPreTrainedModel):
    ...
    def __init__(self, config: CLIPSegConfig):
        ...
        self.visual_adapter = Adapter(768, 4).to(self.clip.dtype)
        self.text_adapter = Adapter(512, 4).to(self.clip.dtype)
        
        self.tunable_linear = torch.nn.Linear(512, 1000, bias=False)
    ...
    def forward(
        ...
        for i in range(len(activations)):
            activations[i] = self.visual_adapter(activations[i]) * 0.2 + activations[i] * 0.8
